...
- [ ] 稳定 7 天后再单独评估启用 HSTS；首版迁移不在 `_headers` 中启用 HSTS。
...
仓库实际有四类实现，迁移成本不同，不能把所有 widget 函数都视为 `_shared/widget-common.ts` 统一覆盖：
...
- **首版不启用 `Strict-Transport-Security`**。HSTS 一旦被浏览器记录，回滚到 HTTP 或错误子域配置会变困难；仅在 Cloudflare TLS 和所有相关子域稳定 7 天后，作为独立变更启用。
...
理由：`wrangler-action@v3` 可能跟随 latest wrangler，未来 wrangler 4.x 一旦有 breaking change，部署会突然失败。锁定到具体版本，升级时显式 PR。Sentry sourcemap 步骤使用 `npx @sentry/cli@2.58.2` 或把 `@sentry/cli` 加入 devDependency 由 lockfile 固定，二选一。
...
| HSTS 混入首版 `_headers` | TLS/DNS 还未稳定时浏览器锁定 HTTPS，回滚复杂 | 首版不启用；仅在 Cloudflare TLS 和所有相关子域稳定 7 天后单独 PR 启用 |
