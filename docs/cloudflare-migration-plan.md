...
| CORS | 多个 Supabase Edge Function 有 CORS/origin 判断 | 迁移时要全量审查 `supabase/functions/**`，不只改 `transcribe` |
      ...
      - name: Sentry source maps (disabled by default)
        ...
        run: |
          if find dist/browser -name '*.map' -type f | grep -q .; then
            npx @sentry/cli sourcemaps inject dist/browser
            npx @sentry/cli sourcemaps upload dist/browser \
              --org "$SENTRY_ORG" \
            ...
            # 否则 ngsw.json 内的资源 hash 会和最终部署 JS 不一致。
...
手动触发说明：`workflow_dispatch` 默认只执行 build/test/guards，不部署。只有在 `main` 分支手动运行并设置 `deploy=true` 时才部署 production。
...
```text
Angular build 生成 dist/browser 与 ngsw.json
...
Cloudflare Pages 不作为 Supabase API 代理，浏览器仍直连 Supabase REST/Auth/Storage/Edge Functions。

需要改的是手写 CORS allow-list。不能只改 `transcribe`，必须全量审查所有浏览器或 TWA WebView 会直接调用的 Edge Function。

当前仓库至少需要检查：

- `supabase/functions/transcribe/index.ts`
- `supabase/functions/virus-scan/index.ts`
- `supabase/functions/widget-black-box-action/index.ts`
- `supabase/functions/_shared/widget-common.ts`
- 通过 `_shared/widget-common.ts` 复用 CORS 的 `widget-register`、`widget-summary`、`widget-notify`、`widget-focus-action`
...
- 加入受控 Pages preview，例如 `https://nanoflow.pages.dev` 和 `https://pr-123.nanoflow.pages.dev` 这类模式。
- 保留 `https://dde-eight.vercel.app` 作为 24-72 小时回滚窗口。
- 同步更新所有 contract tests，不只更新 `src/tests/contracts/transcribe-cors.contract.spec.ts`。

注意：Supabase Auth Redirect URLs 支持 globstar，但你自己写的 Edge Function CORS allow-list 不会自动理解 `https://pr-*.nanoflow.pages.dev`。如果代码是普通字符串比较，这个字符串永远不会匹配真实 preview 域名。应使用 hostname 解析或正则。
...
- [ ] 新增 `.github/workflows/deploy-cloudflare-pages.yml`。
- [ ] 明确 `workflow_dispatch` 行为：默认只 build/test；只有 `deploy=true` 且分支为 `main` 才生产部署。
- [ ] PR preview deploy 条件限制为同仓库 PR，不使用 `pull_request_target`。
- [ ] 全量审查 `supabase/functions/**` 的 CORS/origin 判断，不只更新 `transcribe`。
- [ ] 更新所有相关 contract tests。
- [ ] 更新 `SentryLazyLoaderService` 的 `tracePropagationTargets`，纳入新 custom domain 和 Pages preview 域。
- [ ] CI artifact guard 补齐 `ngsw-worker.js`、`manifest.webmanifest`、`.well-known/assetlinks.json`、TWA package name、`.map` 最终门禁。
- [ ] 如启用 Sentry Source Map，新增 hidden source map 构建配置，并确保 inject 后执行 `npx ngsw-config dist/browser ngsw-config.json /`。
- [ ] 保留 `npm run perf:guard:nojit` 为部署前门禁。
...
- [ ] 若 sourcemap 开启，确认 `ngsw.json` 是 inject 后重建版本，并确认 Debug ID sourcemap 可还原堆栈。
...
- 如果启用 Sentry sourcemap，`ngsw.json` 必须在 `sourcemaps inject`、`.map` 删除后重建。
...
- PR preview 不污染生产 Supabase，或只执行只读/隔离写入。
...
- Android TWA 的 `assetlinks.json` 返回正确。
...
| Sentry inject 后未重建 `ngsw.json` | Angular Service Worker hash 失配，PWA 安装/更新失败或 version skew | 首版默认关闭 sourcemap；启用时 inject、upload、删 `.map` 后执行 `npx ngsw-config dist/browser ngsw-config.json /` |
