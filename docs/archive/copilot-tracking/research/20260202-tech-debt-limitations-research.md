...
| 任务 | 状态 | 结果 |
|------|------|------|
| 修复未解析导入错误 | ✅ | 修复 store-persistence.service.ts 路径错误 |
| 删除未使用 barrel 文件 | ✅ | 删除 16 个空 index.ts 文件 |
| 删除未使用测试辅助文件 | ✅ | 删除 6 个测试 mock 文件 |
| SentryLazyLoaderService 增强 | ✅ | 添加 addBreadcrumb, setExtra, setMeasurement 等方法 |
| 创建 RPC 批量查询迁移 | ✅ | 20260202_performance_rpc_functions.sql |
| **Sentry 导入统一迁移** | ✅ | **29+ 服务文件迁移完成，0 直接导入** |
| **测试修复** | ✅ | **879 通过，62 跳过，0 失败** |
| **RPC 函数同步到 init 脚本** | ✅ | **init-supabase.sql v3.6.0** |
| **导入路径修复** | ✅ | **4 个服务文件路径修复** |
| **清理迁移脚本** | ✅ | **删除 9 个未使用脚本文件** |

### 最终 Bundle 状态

```
Initial Total: 1.93 MB (目标 2 MB ✅ 达成)
Main Bundle: 620 KB (目标 500 KB, 超出 120 KB)
Sentry Lazy Chunk: 422.23 KB (已从 Initial 移出)
```
...
| 指标 | 优化前 | 优化后 | 收益 |
|------|--------|--------|------|
| Initial Bundle | 2.34 MB | 1.93 MB | **-410 KB** |
| Sentry 位置 | Initial | Lazy | **首屏无 Sentry** |
| 直接 Sentry 导入 | 29+ 处 | 0 处 | **100% 迁移** |
| 测试失败 | 254 | 0 | **全部修复** |
