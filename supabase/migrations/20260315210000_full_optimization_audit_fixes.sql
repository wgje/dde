...
DROP INDEX IF EXISTS public.idx_projects_owner_updated;       -- 被 idx_projects_owner_id_updated 替代
DROP INDEX IF EXISTS public.idx_projects_owner_updated_desc;  -- 被 idx_projects_id_owner_updated_desc 替代

-- black_box_entries 表冗余索引
DROP INDEX IF EXISTS public.idx_black_box_entries_project_id;           -- project_id 已改为可空，旧查询模式
DROP INDEX IF EXISTS public.idx_black_box_entries_user_date;            -- 从未使用
DROP INDEX IF EXISTS public.idx_black_box_entries_user_updated_desc;    -- 被 idx_black_box_entries_user_updated 替代
DROP INDEX IF EXISTS public.idx_black_box_entries_user_shared_updated;  -- 新建但从未使用

-- focus_sessions / routine_tasks 冗余单列索引（被复合索引覆盖）
DROP INDEX IF EXISTS public.idx_focus_sessions_user_id;   -- 被 idx_focus_sessions_user_updated_at 覆盖
DROP INDEX IF EXISTS public.idx_routine_tasks_user_id;    -- 被 idx_routine_tasks_user_updated 覆盖

-- tombstone 表未使用索引
DROP INDEX IF EXISTS public.idx_task_tombstones_deleted_by;              -- 从未使用
DROP INDEX IF EXISTS public.idx_connection_tombstones_deleted_by;        -- 从未使用
DROP INDEX IF EXISTS public.idx_connection_tombstones_project_deleted_desc; -- 从未使用

-- 其他未使用索引
DROP INDEX IF EXISTS public.idx_quarantined_files_quarantined_by;  -- 从未使用
DROP INDEX IF EXISTS public.idx_project_members_invited_by;        -- 从未使用
