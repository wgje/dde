...
describe('widget focus reorder helper', () => {
  it('moves a selected backup secondary to C position #2 while keeping the main task fixed', () => {
    ...
    expect(entries.map(item => item.taskId)).toEqual(['main', 'd', 'a', 'b', 'c', 'done']);
  ...
  it('rejects completed, missing, and main tasks without changing the snapshot', () => {
    ...
    expect(promoteSecondaryTaskToC2(snapshot, 'main', SAVED_AT)).toMatchObject({
      ok: false,
      code: 'MAIN_TASK_FIXED',
    });
