...
@Component({
  ...
  styles: [`
    ...
    .focus-scene {
      ...
      opacity: 0;
      overflow: hidden;
      transition: opacity 0.58s cubic-bezier(0.25, 0.06, 0.20, 1.0);
    }
    .focus-scene.active {
      opacity: 1;
    }
    ...
    .focus-bg-image {
      ...
      filter:
        saturate(var(--scene-base-saturation, 1.08))
        brightness(var(--scene-base-brightness, 0.92));
      animation: focusSceneDrift var(--scene-drift-duration, 24s) ease-in-out infinite alternate;
    }
    .focus-scene-aurora {
      background:
        radial-gradient(circle at 18% 24%, rgba(var(--scene-primary-rgb, 99 102 241), calc(var(--scene-aurora-opacity, 0.18) * 0.95)) 0%, transparent 34%),
        radial-gradient(circle at 76% 30%, rgba(var(--scene-secondary-rgb, 52 211 153), calc(var(--scene-aurora-opacity, 0.18) * 0.82)) 0%, transparent 38%),
        radial-gradient(circle at 54% 78%, rgba(var(--scene-tertiary-rgb, 245 158 11), calc(var(--scene-aurora-opacity, 0.18) * 0.68)) 0%, transparent 44%);
      mix-blend-mode: screen;
      opacity: 0.92;
      filter: blur(var(--scene-aurora-blur, 54px));
    ...
    .focus-scene[data-performance-tier='T1'] .focus-bg-image {
      animation-duration: calc(var(--scene-drift-duration, 24s) * 1.2);
    }
    .focus-scene[data-performance-tier='T1'] .focus-scene-grain {
      opacity: calc(var(--scene-grain-opacity, 0.04) * 0.5);
      animation: none;
    }
    .focus-scene[data-performance-tier='T2'] .focus-bg-image,
    .focus-scene[data-performance-tier='T2'] .focus-scene-aurora,
    .focus-scene[data-performance-tier='T2'] .focus-scene-grain,
    .focus-scene[data-reduced-motion='true'] .focus-bg-image,
    ...
    .focus-backdrop {
      pointer-events: none;
      opacity: 0;
      transition: opacity 0.52s cubic-bezier(0.25, 0.06, 0.20, 1.0),
                  background-color 0.52s cubic-bezier(0.25, 0.06, 0.20, 1.0);
    }
    ...
    .dock-bar {
      ...
      /* will-change 已移除：CSS transition 声明足以触发浏览器合成层优化，
         常驻 will-change 白白占用 GPU 显存（尤其移动端） */
    }
        ...
        @if (dockSecondaryRailActive() || bannerPlannerTarget()) {
          <div
            ...
            @if (bannerPlannerTarget(); as bannerTarget) {
              <button
                type="button"
                class="flex items-center gap-1.5 rounded-full border border-amber-400/25 bg-amber-500/10 px-2 py-0.5 cursor-pointer transition-colors hover:bg-amber-500/20"
                [class.ml-auto]="dockSecondaryRailActive()"
          ...
          @for (entry of engine.orderedDockEntries(); track entry.taskId) {
            <div
              ...
              @if (canUsePlannerQuickEdit()) {
                <button
                  type="button"
                  class="mt-2 w-full text-[10px] text-slate-500 hover:text-slate-300 transition-colors rounded px-1 py-0.5"
