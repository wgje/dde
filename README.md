...
- **Android 小组件**：Gate 列表改用更稳的 RemoteViews 渲染；bootstrap 回跳更安全；Focus 退出后也会直推摘要
- **同步与稳定性**：Focus/Dock 云端拉取超时保持本地快照，浏览器网络挂起错误降噪；主线仍以 LWW 作为最终一致性基线
- **Cloudflare 迁移推进**：Cloudflare Pages 迁移方案与部署门禁已整理，CI 产物 guard/趋势基线补齐（见文档导航）
- **CI/测试**：分片基线与隔离修复，部署 workflow rerun/统计更稳
