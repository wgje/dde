...
| 重试中 | 上次推送失败 | 检查网络/登录状态，必要时看系统仪表盘 |
...
| 任务有四个优先级 | low / medium / high / urgent，方便排序 |
...
近期已修复项目元数据刷新后的 retry handoff 和 terminal failure 清理。若仍出现，先确认登录状态、网络和 Supabase RPC 是否部署完整；随后打开**设置 → 系统仪表盘**，查看是否有 RetryQueue 死信或断路器冷却。
